...
function footer() {
  return `<footer class="site-footer"><div class="footer-top"><div><a class="brand footer-brand" href="/" data-link><img src="/assets/cherry-clean-logo.jpeg" alt="Cherry Clean Services LLC" /></a><p class="footer-statement">A cleaner kind of confidence.<br />For homes, businesses, and everything in between.</p><a class="yelp-footer-link" href="${YELP_URL}" target="_blank" rel="noopener noreferrer" aria-label="Review Cherry Clean Services LLC on Yelp">${yelpMark()}<span>Review us</span></a></div><div class="footer-links"><span>Explore</span><a href="/services/" data-link>Services</a><a href="/about/" data-link>Our story</a><a href="/contact-us/" data-link>Contact</a></div><div class="footer-links"><span>Reach us</span><a href="tel:${PHONE_LINK}">${PHONE}</a><a href="mailto:${EMAIL}">${EMAIL}</a><a class="yelp-footer-text-link" href="${YELP_URL}" target="_blank" rel="noopener noreferrer">Yelp business page</a><span>Concord, CA 94521</span></div></div><div class="footer-bottom"><span>Cherry Clean Services LLC</span><span>Serving Concord & nearby Bay Area cities</span><span>© ${new Date().getFullYear()} All rights reserved</span></div></footer>`
}

function yelpButton(label = 'Find us on Yelp', accessibleLabel = label, variant = '') { return `<a class="button yelp-button ${variant}" href="${YELP_URL}" target="_blank" rel="noopener noreferrer" aria-label="${accessibleLabel}">${yelpMark()}<span>${label}</span>${icon('arrow')}</a>` }
