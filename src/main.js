...
function header() {
  return `<header class="site-header"><a class="brand" href="/" data-link aria-label="Cherry Clean Services LLC home"><img src="/assets/cherry-clean-logo.jpeg" alt="Cherry Clean Services LLC logo" /></a><nav class="desktop-nav">${nav.map(([label, path]) => `<a href="${path}" data-link>${label}</a>`).join('')}</nav><a class="header-cta" href="/contact-us/" data-link>Request a quote ${icon('arrow')}</a><a class="yelp-header-link" href="${YELP_URL}" target="_blank" rel="noopener noreferrer" aria-label="Visit Cherry Clean Services LLC on Yelp">Yelp</a><button class="menu-toggle" aria-label="Open menu">${icon('menu')}</button><div class="mobile-menu"><button class="menu-close" aria-label="Close menu">${icon('close')}</button>${nav.map(([label, path]) => `<a href="${path}" data-link>${label}</a>`).join('')}<a class="mobile-quote" href="/contact-us/" data-link>Request a quote ${icon('arrow')}</a><a class="mobile-yelp-link" href="${YELP_URL}" target="_blank" rel="noopener noreferrer">Find us on Yelp</a></div></header>`
}

function footer() {
  return `<footer class="site-footer"><div class="footer-top"><div><a class="brand footer-brand" href="/" data-link><img src="/assets/cherry-clean-logo.jpeg" alt="Cherry Clean Services LLC" /></a><p class="footer-statement">A cleaner kind of confidence.<br />For homes, businesses, and everything in between.</p><a class="yelp-footer-link" href="${YELP_URL}" target="_blank" rel="noopener noreferrer">Review us on Yelp</a></div><div class="footer-links"><span>Explore</span><a href="/services/" data-link>Services</a><a href="/about/" data-link>Our story</a><a href="/contact-us/" data-link>Contact</a></div><div class="footer-links"><span>Reach us</span><a href="tel:${PHONE_LINK}">${PHONE}</a><a href="mailto:${EMAIL}">${EMAIL}</a><a href="${YELP_URL}" target="_blank" rel="noopener noreferrer">Yelp business page</a><span>Concord, CA 94521</span></div></div><div class="footer-bottom"><span>Cherry Clean Services LLC</span><span>Serving Concord & nearby Bay Area cities</span><span>© ${new Date().getFullYear()} All rights reserved</span></div></footer>`
}

function yelpButton(label = 'Find us on Yelp') { return `<a class="button yelp-button" href="${YELP_URL}" target="_blank" rel="noopener noreferrer">${label} ${icon('arrow')}</a>` }
...
function resultsGallerySection() { return `<section class="results-showcase section-pad"><div class="section-kicker">04 / Finished results</div><div class="results-showcase-heading"><div><h2>Results that speak<br /><em>for themselves.</em></h2><p>Every detail counts. Our goal is to leave spaces clean, fresh, and completely renewed.</p></div>${yelpButton('See us on Yelp')}</div><div class="results-masonry">${resultsGallery.map(([file, alt], index) => `<figure class="result-tile result-tile-${index + 1}"><img src="/assets/results/${file}" alt="${alt}" loading="eager" decoding="async" /><figcaption>${alt}</figcaption></figure>`).join('')}</div></section>` }
...
function render() {
  ...
  if (path === '/') content = content.replace('</main>', `${resultsGallerySection()}</main>`)
  const rendered = `${header()}${content}${footer()}`
    ...
    .replace('<section class="proof-strip">', `${yelpButton('Read our Yelp reviews')}<section class="proof-strip">`)
